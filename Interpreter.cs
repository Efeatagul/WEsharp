...
namespace WSharp
{
    public class Interpreter : IVisitor<WValue>, IStmtVisitor<object>
    {
        ...
        public Interpreter()
        {
            ...
            LoadLibrary(new SoundLib().GetFunctions());
            LoadLibrary(new InputLib().GetFunctions());
            LoadLibrary(new AdvancedLibrary().GetFunctions());
        ...
        {
            foreach (var func in lib)
            {
                _scope.Define(func.Key, new WValue(func.Value));
            }
        }
        ...
        {
            WValue val = stmt.Initializer != null ? Evaluate(stmt.Initializer) : new WNull();
            _scope.Define(stmt.Name.Value, val);
            return null;
        }
        ...
        private void ExecuteBlock(List<Stmt> statements, Scope nextScope)
        {
        ...
        public object Visit(WhileStmt stmt)
        {
            ...
            {
                Execute(stmt.Body);
                if (_breakingLoop) { _breakingLoop = false; break; }
            }
            return null;
        }
        ...
        {
            _functions[stmt.Name.Value] = stmt;
            return null;
        }
        ...
        public object Visit(TryStmt stmt)
        {
            try
            {
                ExecuteBlock(stmt.TryBlock, new Scope(_scope));
            }
            catch
            {
                if (stmt.CatchBlock != null) ExecuteBlock(stmt.CatchBlock, new Scope(_scope));
            }
            return null;
        }
        ...
        public WValue Visit(VariableExpr expr)
        {
            if (_functions.ContainsKey(expr.Name.Value)) return new WValue("FUNCTION", WType.String);
            return _scope.Get(expr.Name);
        }

        public WValue Visit(AssignExpr expr)
        {
            WValue val = Evaluate(expr.Value);
            _scope.Assign(expr.Name, val);
            return val;
        }
            ...
            switch (expr.Operator.Value)
            {
                case "+":
                    if (left.Type == WType.String || right.Type == WType.String)
                        return new WString(left.AsString() + right.AsString());
                    return new WNumber(left.AsNumber() + right.AsNumber());
        ...
        public WValue Visit(CallExpr expr)
        {
            ...
            if (expr.Callee is VariableExpr v && _functions.ContainsKey(v.Name.Value))
            {
                ...
                try
                {
                    ExecuteBlock(func.Body, funcScope);
                }
                catch { }
            ...
            throw new Exception("Bilinmeyen fonksiyon cagrisi.");
        }

        private bool IsTruthy(WValue val)
        {
            if (val.Type == WType.Null) return false;
            if (val.Type == WType.Number) return val.AsNumber() != 0;
            if (val.Type == WType.String) return val.AsString() != "false";
            return true;
        }
