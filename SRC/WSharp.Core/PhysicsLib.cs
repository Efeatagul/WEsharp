...
namespace WSharp
{
    ...
    public class PhysMechFunc : IWCallable
    {
        ...
        public WValue Call(Interpreter interpreter, List<WValue> args)
        {
            ...
            if (mode == "motion") return new WValue(Mechanics.Motion(0, v1, v2)); 
            if (mode == "force") return new WValue(Mechanics.Force(v1, v2));
            if (mode == "projectile") return new WValue(Mechanics.Projectile(v1, v2));
            if (mode == "gravity") return new WValue(Mechanics.Gravitation(v1, v2, 1)); 
    ...
    public class PhysEMFunc : IWCallable
    {
        ...
        public WValue Call(Interpreter interpreter, List<WValue> args)
        {
            ...
            if (mode == "coulomb") return new WValue(Electromagnetism.Coulomb(v1, v2, 1));
            if (mode == "ohm") return new WValue(Electromagnetism.Circuit(v1, v2));
            if (mode == "lorentz") return new WValue(Electromagnetism.Lorentz(v1, v2, 1)); 
    ...
    public class PhysModernFunc : IWCallable
    {
        ...
        public WValue Call(Interpreter interpreter, List<WValue> args)
        {
            ...
            if (mode == "relativity") return new WValue(ModernPhysics.Relativity(1, v1)); 
            if (mode == "photoelectric") return new WValue(ModernPhysics.PhotoElectric(v1, 2.0)); 
